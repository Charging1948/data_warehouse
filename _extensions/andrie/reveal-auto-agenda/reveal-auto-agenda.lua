	...
	for _, block in pairs(blocks) do
		...
		then
			...
			if options_heading ~= nil then
				newBlocks:insert(pandoc.Div(pandoc.Para(options_heading), pandoc.Attr("", { "agenda-heading" })))
			end
			...
			for i = 1, #headers do
				...
				mod_headers:insert(pandoc.Div(pandoc.Para(headers[i]), pandoc.Attr("", agenda_class)))
			end

			-- insert the agenda items
			newBlocks:insert(pandoc.Div(bullet_class(mod_headers), pandoc.Attr("", { "agenda" })))
		else
			if block.attr ~= nil and block.t == "Header" and block.attr.classes:includes("no-auto-agenda") then
			else
				newBlocks:insert(block)
			end
		end
